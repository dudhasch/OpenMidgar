...
| K7 | **Tile-z → Sichtdistanz** | `viewDistance = max(1, z · zScale)`, **Default `zScale = 4`** | 🟡 belegte untere Schranke, kein Beweis | Zwei Messungen. (a) S9 über 666 Fields: zwischen `z` (u16@26) und der Sichtdistanz gibt es **keinen** konstanten Faktor (Streuung p10…p90 über drei Größenordnungen) — `z` ist ein Sortierschlüssel, keine Metrik. (b) S11-Ordnungsbedingung über alle 702 Fields: Layer 0 trägt in **allen** 342.792 Tiles exakt 4095 und ist die hinterste Ebene, also muss `4095·zScale` jede begehbare Sichtdistanz übertreffen. Der nötige Faktor liegt im Median bei 0,66, p99 = 3,31, **Maximum 3,77** — bei `zScale = 1` erfüllen nur 476/702 Fields die Bedingung, bei 4 alle. Dass ausgerechnet eine Zweierpotenz knapp aufgeht, ist ein starkes Indiz; entschieden wird erst durch die Sichtprüfung der Figurenverdeckung |
