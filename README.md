...
- ✅ **ADR-028 + Sprachzweig**: Die eigene Ghidra-Analyse der PC-EXE ist ohne Auflagen freigegeben (der Bestand selbst wird **nicht** eingecheckt), und daraus fiel sofort ein Fehler auf, den niemand gesucht hatte: Das Projekt las durchgehend `data/kernel/` und `data/battle/` — auf dieser Installation den **deutschen** Zweig, während das Original `data/lang-en/` lädt. Entschieden hat eine Kreuzvalidierung, kein Größenvergleich: Der Blockindex in `kernel.bin`+0x0F1C trifft die Größe der `scene.bin` *desselben* Zweigs byteexakt (34×8192 = 278.528 und 33×8192 = 270.336), die Kreuzpaarung scheitert. Alle 28 abweichenden Byte der Sektion 2 liegen in dieser Tabelle — der Unterschied ist keine Übersetzung, sondern eine Packungsfolge. Demo und alle 20 betroffenen Proben umgestellt; in `scene.bin` ändert das **nur Text** plus die eine leergeräumte Szene 4 (−2 Gegnerrecords, −2 KI-Skripte, −13 Formationsplätze), **K8 bleibt unberührt**. Dazu: `0xFFFF` bei @56/@58 heißt „noch nicht berechnet" und nicht 65535 — in **24 von 63** echten Records, lesend *und* schreibend wirksam
- 🔶 **Welle 5 — Kampf** (laufend): **K8** vermessen und die Diagnose von F26 dadurch **ausgetauscht**. Vier Gütefunktionen sind gescheitert (Bedarfswinkel 17,65° gegen Kontrollen 15,5°; Punkt-Fit gegen zwei im Original vermessene Schattenmitten — echt **17,51 px**, beste *fremde* Kamera **3,50 px**; Bildkorrelation der Bühne max 0,14; Bildfüllung flach 90–97 %). Entschieden hat keine bessere Statistik, sondern eine **Invariante**: Ein Öffnungswinkel skaliert radial um die Bildmitte und kann keine *Richtung* ändern (gerechnet auf 2,7·10⁻¹⁵ genau). Am gelben Bühnenschild liegt das Richtungsverhältnis der Originalaufnahme bei **1,381**, das der Blockkameras konstant bei **0,826** bzw. **2,81** ⇒ 🟢 **keine der drei Blockkameras zeigt die Ansicht des Originals, bei keinem Winkel**. Die Frage ist damit nicht „welcher Zoom", sondern „welche Kamera" — **K11 hat `camdat` aufgeschlossen**: vier PSX-absolute Zeiger gegen `0x801A0000`, fünf Invarianten, in allen drei Dateien byteexakt (394/375/374 Takes, 2364 Verzeichnisplätze auf 1020 **geteilte** Körper); Kontrolle **0 von 18** falschen Zeigerbasen, auch die um nur 4 Byte. Offen bleiben die Opcodes der beiden Kamera-VMs. **K10** hat `ff7-fenrir`/`Braver`/`kujata` gesichtet und dabei zwei Felder des Gegnerrecords entschieden: `+0xB0` sind die **erlaubten** Zustände, nicht die Immunitäten (wir hatten das Vorzeichen falsch — 26,2 von 32 Bits gesetzt, ein Drittel `0xFFFFFFFF`), und `+0xA2` ist der Rückenangriffs-Faktor in Achteln (602/625 auf ×2,0; Kontrolle: 2 von 184 Byteversätzen erfüllen die Achtel-Vorhersage). **K9** ist ein Negativbefund mit gesicherter Suchmenge: Das Field-`.a`-Accounting trifft 0 von 872 — das Format ist ausgeschlossen, die Bindpose bleibt. Nebenbefund: Die Referenzaufnahme ist **Formation 301** (2×MP, EXP 32/AP 4, aus den Encounter-Tabellen des Bahnhofsbogens), und ihr Bühnenrendering liefert den bislang fehlenden **Sichtnachweis für K5** (`Präfixindex = 370 + location`)
