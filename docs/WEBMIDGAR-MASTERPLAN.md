...
**Rechtsrahmen:** Es werden keine Originalassets, Originaldialoge oder Bytecode-Dumps verteilt oder eingebettet. Alle Spieldaten stammen ausschließlich aus einer lokal vom Nutzer bereitgestellten, legal erworbenen Installation und verlassen den Browser nie. Formatwissen stammt aus öffentlich dokumentierter Community-Forschung (Qhimm Wiki, q-gears/ffvii-tools, Makou Reactor, TouphScript) und ist grundsätzlich als **zu verifizierende Referenz** zu behandeln. Seit [ADR-027](ADR-027-DECOMP-REFERENZ.md) zählen dazu auch dekompilierte **Fremd**quellen — als Referenz lesbar, mit Herkunftspflicht, Plattformvorbehalt und **ohne Textübernahme** (die betreffenden Repos stehen ohne Lizenz). Für die **eigene** Analyse der PC-EXE gilt seit [ADR-028](ADR-028-EIGENE-CODEANALYSE.md) keine dieser Auflagen; der Bestand selbst wird nicht eingecheckt.
...
Ein sicherer, deterministischer, unterbrechbarer Interpreter für Field-Events. Der Interpreter führt den **im Nutzerdatenbestand vorhandenen** Bytecode aus; WebMidgar definiert nur die Ausführungssemantik. Semantiktabellen werden **vorrangig aus den Daten selbst** abgeleitet (Gütefunktion + Kontrollhypothese, s. O9) und erst nachrangig aus Fremdquellen; seit [ADR-027](ADR-027-DECOMP-REFERENZ.md) gehören dekompilierte Fremdquellen dazu, unter Herkunftspflicht und Messvorrang (Auflagen A2/A4) und ohne Übernahme von Quelltext (A1). Für die eigene Analyse der PC-EXE gilt seit [ADR-028](ADR-028-EIGENE-CODEANALYSE.md) keine dieser Auflagen — der Messvorrang bleibt dort Qualitätsmaßstab, nicht Regel.
...
| ADR-028 | Die **eigene** Ghidra-Analyse von `ff7_en.exe` (PC) ist vollständig und **ohne Auflagen** freigegeben; S37-Zusatzregel 4 („kein Disassemblieren") ist aufgehoben. Die vier Auflagen aus ADR-027 gelten für diesen Bestand nicht — er ist erstbeteiligt an einer eigenen Kopie erhoben, es gibt keinen Dritten mit Rechten am Text. Volltext: [ADR-028-EIGENE-CODEANALYSE.md](ADR-028-EIGENE-CODEANALYSE.md) | Zusatzregel 4 nur präzisieren (Datenextraktion ja, Codeanalyse nein); Freigabe auf `spec/` beschränken und `pseudocode/` sperren | Öffnet Schadensformeln/ATB, Statusnumerierung, Zufallsgeneratoren, `camdat` (K11), `da`/`ab` (K9), Minigame-Physik; Rechtsposition wird ein zweites Mal schwächer und ist diesmal dem Projekt selbst zurechenbar. **Unberührt:** Beweisklassenordnung (🟡 bis Gegenprobe), „Fundstelle statt Fundstück", BYO-Data — der Bestand wird nicht eingecheckt | Akzeptiert |
